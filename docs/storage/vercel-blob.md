...
- `PATCH /api/blob/chapters`
- `DELETE /api/blob/chapters?id=<chapterId>`
- Chapters are stored in `Chapter`, keyed by Blob pathname.
- `label` is required, `seconds` must be non-negative, and `color` is an optional hex color.
- When the client sends `durationSeconds`, the API validates chapter position and syncs `Video.durationSeconds` for that pathname.
