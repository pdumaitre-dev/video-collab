...
- `Comment_blob`: comment ranges keyed by Blob pathname. Replies use nullable `parentId` self-relations and inherit the parent comment's time range.

Current UI behavior uses `Video` and `Comment_blob`. The older `Comment` model is still present in the schema, but the active Blob-backed flow does not read from it. The timeline renders ranges for top-level `Comment_blob` rows only; nested replies stay inside the expanded comment thread.
