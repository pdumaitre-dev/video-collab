...
- `Comment_blob`: comment ranges keyed by Blob pathname. Optional `parentId`
  links replies to another Blob comment; replies inherit the parent range, and
  timeline markers use only top-level comments.
