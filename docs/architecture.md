...
4. `app/videos/watch/[filename]/FileVideoPageShell.tsx` loads comments and chapters through Blob pathname-keyed APIs.
...
- `app/api/blob/chapters/route.ts`: pathname-keyed chapter read/write/update/delete API.
...
- `Chapter`: named single-time timeline bookmarks keyed by Blob pathname.
...
- `components/TimeBar.tsx` exposes one shared horizontal scale for ruler ticks, chapter markers, and the seek bar so drag-to-select can begin on either surface and end anywhere on the page.
