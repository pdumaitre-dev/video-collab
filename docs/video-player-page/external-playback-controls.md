...
- The comments panel shows a **Loop range** toggle when a loop target exists; it defaults on.
...
- Pausing stops loop advancement until play resumes; the toggle stays armed.
- Looping is client-only. No API or schema changes are required. See `loop-selected-range.md`.
