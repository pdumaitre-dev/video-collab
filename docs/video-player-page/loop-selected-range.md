...
The loop target is resolved in `VideoPageShell`:
...
2. **Draft range** — dragging a range on `TimeBar` for a new comment clears the selected comment and loops that yellow selection until submit or another selection replaces it.
...
The **Loop range** checkbox in the comments panel header defaults to on. When off, comment clicks only seek once and new range selections do not start playback.
